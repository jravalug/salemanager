...
def export_test(business_id):
    repository = SalesRepository()
    # Obtener el negocio
    business = Business.query.get_or_404(business_id)
    selected_month = datetime.strptime("2025-01", "%Y-%m").date()
    start_date = selected_month.replace(day=1)
    end_date = start_date + relativedelta(months=1, days=-1)
    excluded_sales = Sale.query.filter(
    ...
    for sale in excluded_sales:
        total_saled += sale.total_amount

    # Enviar el archivo como respuesta
    return render_template_string(f"El total de ventas excluidas fue de: {total_saled}")
